				...
				<tr>
					...
					<td rowspan="3"><div class="box big grey" style="background-color:#F2F2F2;">
						...
						<table style="width: 100%;">
							<tr>
								<td rowspan="3" style="width: 50%;"><h3>${array[6]}</h3></td>
								<td>이상</td>
								<td>0 건</td>
							</tr>
							<tr>
								<td colspan="2"><hr color="darkgrey"/></td>
							</tr>
							<tr>
								<td>의심</td>
								<td>0 건</td>
		...
		<tr>
			...
			<div class="d-tb"><form action=""><table>
				...
				<tr>
					...
					<td>
						<c:if test="${t_dto.getTran_ab().equals('02') }">이상</c:if>
						<c:if test="${t_dto.getTran_ab().equals('03') }">의심</c:if>				
					</td>
					<td>
						<c:if test="${t_dto.getTran_chn().equals('01') }">영업점</c:if>
						<c:if test="${t_dto.getTran_chn().equals('02') }">자동화기기</c:if>
						<c:if test="${t_to.getTran_chn().equals('03') }">인터넷뱅킹</c:if>
						<c:if test="${t_dto.getTran_chn().equals('04') }">스마트뱅킹</c:if>
						<c:if test="${t_dto.getTran_chn().equals('05') }">제휴</c:if>
						<c:if test="${t_dto.getTran_chn().equals('06') }">오픈API</c:if>
						<c:if test="${t_dto.getTran_chn().equals('07') }">공동망</c:if>
					</td>
					<td>
						<c:if test="${t_dto.getTran_name().equals('01') }">계좌신규</c:if>
						<c:if test="${t_dto.getTran_name().equals('02') }">자동이체</c:if>
						<c:if test="${t_dto.getTran_name().equals('03') }">가상계좌발급</c:if>
						<c:if test="${t_dto.getTran_name().equals('04') }">가상계좌입금</c:if>
						<c:if test="${t_dto.getTran_name().equals('05') }">대출신청</c:if>
					</td>	
					<td>${t_dto.getTran_date()}</td>
					<td>
						<c:if test="${t_dto.getTran_act().equals('00') }">미확인
						...
						<c:if test="${t_dto.getTran_act().equals('02') }">대응완료</c:if>
					</td>		
				</tr>
				</c:forEach>
				</c:if>						
			</table><br /><br /><br /></form></div>
			</td>
		</tr>
		</table>
		</div>				
